...
run_and_capture "Easy install support summary" "$BUNDLE_DIR/tv-box-easy-summary.txt" env BOX_IP="$BOX_IP" DEVICE_SERIAL="$DEVICE_SERIAL" PACKAGE_NAME="$PACKAGE_NAME" EASY_RUN_RESULT="${EASY_RUN_RESULT:-support_bundle}" TV_BOX_INSPECTION_JSON="$REPORT_DIR/tv-box-inspection-latest.json" TV_BOX_EASY_SUMMARY_MD="$SUPPORT_EASY_SUMMARY_MD" TV_BOX_EASY_SUMMARY_JSON="$SUPPORT_EASY_SUMMARY_JSON" npm run -s tv-box:easy-summary
