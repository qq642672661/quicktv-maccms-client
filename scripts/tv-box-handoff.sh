...
4. 安装前双击 SITE_READINESS_CARD.html，看当前应先发包、授权、安装还是补证据。
...
如果不习惯看 Markdown，直接双击 \`START_HERE.html\`；只想教家人怎么用时双击 \`OPERATION_CARD.html\` 打印；需要采购/复核硬件时双击 \`HARDWARE_SELECTION_CARD.html\` 打印；测完真实盒子准备发回证据时双击 \`FIELD_RETURN_CARD.html\`，再把文件放进 \`FIELD_RETURN\` 文件夹并双击 \`PACK_FIELD_RETURN_ON_WINDOWS.bat\` 或 \`PACK_FIELD_RETURN_ON_MAC.command\` 打包；如果电脑打不开网页文件，就打开 \`README_FIRST.txt\`。
