...
C920_CONFIRM_JSON="$REPORT_DIR/tv-box-c920-confirm-latest.json"
...
PHONE_CAMERA_CAPTURE_JSON="$REPORT_DIR/tv-box-phone-camera-capture-test-latest.json"
PHONE_CAMERA_CAPTURE_MD="$REPORT_DIR/tv-box-phone-camera-capture-test-latest.md"
...
(cd "$ROOT_DIR" && npm run -s tv-box:c920-confirm >/dev/null)
(cd "$ROOT_DIR" && npm run -s tv-box:c920-prep >/dev/null)

(cd "$ROOT_DIR" && npm run -s tv-box:phone-camera-contract >/dev/null)

(cd "$ROOT_DIR" && npm run -s tv-box:phone-camera-capture-test >/dev/null)

(cd "$ROOT_DIR" && npm run -s tv-box:phone-camera-readiness >/dev/null)
...
if [[ -f "$C920_CONFIRM_JSON" ]]; then
  cp "$C920_CONFIRM_JSON" "$HANDOFF_DIR/tv-box-c920-confirm-latest.json"
fi

if [[ -f "$C920_CONFIRM_MD" ]]; then
  cp "$C920_CONFIRM_MD" "$HANDOFF_DIR/tv-box-c920-confirm-latest.md"
fi
...
if [[ -f "$PHONE_CAMERA_CAPTURE_JSON" ]]; then
  cp "$PHONE_CAMERA_CAPTURE_JSON" "$HANDOFF_DIR/tv-box-phone-camera-capture-test-latest.json"
fi

if [[ -f "$PHONE_CAMERA_CAPTURE_MD" ]]; then
  cp "$PHONE_CAMERA_CAPTURE_MD" "$HANDOFF_DIR/tv-box-phone-camera-capture-test-latest.md"
fi
...
if [[ -f "$PHONE_CAMERA_READINESS_JSON" ]]; then
  cp "$PHONE_CAMERA_READINESS_JSON" "$HANDOFF_DIR/tv-box-phone-camera-readiness-latest.json"
fi

if [[ -f "$PHONE_CAMERA_READINESS_MD" ]]; then
...
cat > "$HANDOFF_DIR/MANIFEST.json" <<MANIFEST
{
  ...
  "reports": {
    ...
    "c920ConfirmMarkdown": "tv-box-c920-confirm-latest.md",
    ...
    "phoneCameraCaptureMarkdown": "tv-box-phone-camera-capture-test-latest.md",
  ...
  "commands": {
    ...
    "phoneCameraCaptureRegression": "npm run tv-box:phone-camera-capture-test",
    "phoneCameraReadiness": "npm run tv-box:phone-camera-readiness",
...
如果要用手机当电视摄像头，先打开 TV_BOX_PHONE_CAMERA_CONTRACT.zh-CN.md；当前默认路线是手机采集 + 局域网 WebSocket 信令 + 电视盒子原生 WebRTC 接收端，不把手机伪装成系统 Camera2 摄像头。tv-box-phone-camera-capture-test-latest.md 证明手机页面有房间码、getUserMedia、RTCPeerConnection、停止按钮和 HTTPS/WSS 安全提示；tv-box-phone-camera-signaling-test-latest.md 证明房间码和 offer/answer/ICE 信令可跑通，电视首帧、音频和真实停止仍等接收端验收。
  ...
  <main>
    ...
    <section class="panel">
      ...
      <ul>
        ...
        <li>C920 PRO 到货：先打开 <a href="C920_ARRIVAL_CARD.html">C920 到货接入卡</a>，按直插、验收命令、电视真实画面确认、麦克风和热插拔顺序处理；确认后打开 <a href="tv-box-c920-confirm-latest.md">C920 确认写入卡</a>，把 pass/fail 写回验收记录。</li>
        ...
        <li>要用手机当电视摄像头：先打开 <a href="TV_BOX_PHONE_CAMERA_CONTRACT.zh-CN.md">手机摄像头信令与验收合同</a> 和 <a href="tv-box-phone-camera-readiness-latest.md">手机摄像头现场准备度</a>，确认 HTTPS/WSS、房间码和信令入口，再按 WebRTC 房间码、信令、隐私和现场证据边界开发/验收。</li>
...
如果要验证“手机当电视摄像头”，先打开 \`TV_BOX_PHONE_CAMERA_CONTRACT.zh-CN.md\`、\`tv-box-phone-camera-capture-test-latest.md\` 和 \`tv-box-phone-camera-signaling-test-latest.md\`，按合同里的房间码、手机 getUserMedia 权限、WebRTC 信令、电视盒子原生接收端、隐私停止按钮和现场证据清单推进；采集页回归只证明 M4 页面入口，信令回归只证明 M2，真实通过仍必须看电视首帧和音频。这条路线不是系统 Camera2 摄像头，微信小程序 \`live-pusher\` 也必须等主体资质和接口权限通过后再作为入口。
