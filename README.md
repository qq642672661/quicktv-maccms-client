...
- 官方 QuickTVUI 仓库暂未提供手机摄像头 WebRTC 示例；“手机当电视摄像头”按 `docs/TV_BOX_REMOTE_CAMERA_PLAN.zh-CN.md` 的分层路线推进：默认推荐手机采集 + 电视端原生 WebRTC 接收，RTSP/RTMP + IJK 只作为同网预览 MVP，USB/UVC/Camera2 继续作为实体摄像头保底。电视端已经有 `phone_camera_pair` 配对入口，摄像头页按 4 可显示二维码和 6 位房间码；按“打开接收端”会把房间码、手机入口、信令地址和媒体档位传给 Android 原生接收端，接收端会用同一个房间码向局域网信令服务创建 TV 房间并观察手机 hello/offer，实验包还会用可选 WebRTC 媒体引擎创建 answer、发送 ICE、渲染远端视频、接收音频并上报 stats。真实手机权限、电视首帧、音频和停止证据闭环前，这个入口仍只证明配对参数、信令房间、媒体引擎入口和验收边界，不证明真实音视频已经通过。微信小程序 `live-pusher` 可作为手机侧免安装入口，但需要服务类目、主体资质和接口权限审核，不能作为默认无门槛承诺。
...
Logitech C920 PRO / C920 Pro HD 到货并插入 USB 后，优先跑专用接入验收。它会先做遥控器基础冒烟，再跑摄像头冒烟、刷新 `inspect`、写入兼容性矩阵、硬件画像、完成度审计和交付总控；电视上是否看到真实画面、C920 自带麦克风是否可用、拔插后是否稳定，仍会保留人工确认结果，不能用自动 Activity 打开冒充摄像头通过。
...
想提前演练“到货日这条命令会不会进入实体验收”，用 dry-run。它会照常检查日期和 ADB 前置状态，但在真正执行 C920 验收前停住，不会写入实体摄像头验收报告；演练结果会写到 `reports/tv-box-c920-arrived-dry-run-latest.md/json`：
...
当前采购状态也会持久记录在 `tv-box-field-state/c920-procurement.json`，所以 `npm run tv-box:c920-arrival-card` 和 `tv-box:next` 不带环境变量时也会显示“京东自营 / 2026-05-31 / 已采购待到货”。`npm run tv-box:c920-arrival-card` 会把最新 C920 验收报告转成 `reports/tv-box-c920-arrival-card-latest.md/json` 和可打印的 `reports/tv-box-c920-arrival-card.html`，给现场人员按“直插 C920 -> 运行验收命令 -> 电视真实画面确认 -> C920 自带麦克风/独立 USB 麦克风 -> USB 热插拔 -> 回传 support zip”顺序处理。小米盒子只有一个 USB 口时，第一轮只直插 C920，一根线同时测画面和 C920 自带麦克风；直插供电不稳、C920 麦克风不进业务或必须外接会议麦克风时，再换带独立供电 USB Hub。它会明确写出：只有电视上看到 C920 PRO 真实画面，`FIELD_CAMERA_PREVIEW` 才能记 `pass`；只看到 USB 线索、Activity 打开或音频线索都不能冒充业务通过。到货卡还会给现场固定证据文件名：`C920_PREVIEW_TV_SCREEN.jpg/mp4`、`C920_MIC_BUSINESS_INPUT.mp4/txt`、`C920_HOTPLUG_RETEST.jpg/txt`、`SUPPORT_CODE_C920.jpg`，并要求一起回传 `tv-box-c920-pro-acceptance-latest.md/json`，这些文件放进交付包的 `FIELD_RETURN/` 后打包回传；工程侧收到后先跑 `npm run tv-box:return-inbox -- <现场回传目录或zip>`，避免漏掉真实画面、麦克风、热插拔、维护码或本次 C920 验收报告。
...
`tv-box:phone-camera-contract` 会生成 `reports/tv-box-phone-camera-contract-latest.md/json`，把“手机采集 + 电视端原生 WebRTC 接收”的房间码、信令消息、状态机、媒体档位、隐私边界、降级路线和现场验收证据固化成机器可读合同；`tv-box:phone-camera-signaling` 可在同网电脑上启动局域网 WebSocket 信令服务，默认监听 `0.0.0.0:17891`，手机采集端和电视接收端都接这个 `/phone-camera/signaling` 入口；现场有 HTTPS 域名/反代/隧道时用 `PHONE_CAMERA_PUBLIC_BASE_URL=https://你的域名/phone-camera npm run tv-box:phone-camera-signaling` 启动，服务会把手机扫码入口和 `wss://.../phone-camera/signaling` 一起发给电视端。电视端二维码入口由 `VITE_PHONE_CAMERA_PAIR_BASE_URL` 配置，媒体档位由 `VITE_PHONE_CAMERA_PROFILE_ID` 配置，打开原生接收端时会传入房间码、手机入口、信令地址和档位，接收端会用同一个房间码发送 `room.create`，让手机扫码页可加入同一个 TV 房间；`tv-box:phone-camera-capture-test` 会生成 `reports/tv-box-phone-camera-capture-test-latest.md/json`，验证 `/phone-camera?room=xxxxxx` 页面已经具备房间码、`getUserMedia`、`RTCPeerConnection`、`webrtc.offer`、停止按钮和 HTTPS/WSS 安全上下文保护；`tv-box:phone-camera-readiness` 会生成 `reports/tv-box-phone-camera-readiness-latest.md/json` 和可打印的 `reports/tv-box-phone-camera-onsite-card.html` / 交付包 `PHONE_CAMERA_ONSITE_CARD.html`，用当前 `VITE_PHONE_CAMERA_PAIR_BASE_URL` 或 `TV_BOX_PHONE_CAMERA_PAIR_BASE_URL` 做现场准备度检查，并把“启动信令 -> 手机 DNS/HTTPS 首开 -> 电视打开配对页 -> 手机扫码 -> 权限 -> 电视首帧/音频/stats/重连/停止证据”压成一页操作卡；默认 `https://quicktv.local/phone-camera` 只是安全占位域名，报告会写入 `provesFieldPhoneReachability:false` 和 `requiresPhoneDnsHttpsProof:true`，并在没有真实手机 DNS、HTTPS 证书和扫码首开证据前给出 warn；如果确实只做实验 HTTP 调试，必须显式加 `TV_BOX_PHONE_CAMERA_ALLOW_INSECURE=true`，否则非 HTTPS 局域网入口会被标为失败。`tv-box:phone-camera-lan-experiment` 会自动用当前 Mac 局域网 IP 构建 `ENABLE_PHONE_CAMERA_WEBRTC=true` 实验 APK、安装到 `BOX_IP`、启动本机信令服务，并要求实机报告里同时出现原生 WebRTC 媒体引擎和 TV 信令房间；它只证明电视接收端实验链路，不代替真实手机权限、首帧、声音和停止证据。`tv-box:phone-camera-signaling-test` 会启动本地局域网 WebSocket 信令服务并生成 `reports/tv-box-phone-camera-signaling-test-latest.md/json`，验证电视端指定房间码创建、HTTPS public base 自动生成 WSS `signalingUrl` 和 `secureContext` 判定、重复房间码拒绝、无效房间拒绝、一台电视配一台手机、`offer` / `answer` / `ice-candidate` / `keepalive` 转发、`session.stats` 接收和挂断关闭房间；`tv-box:phone-camera-scenarios-test` 会生成 `reports/tv-box-phone-camera-scenarios-test-latest.md/json`，用合成信令回归扫码首帧、房间过期、手机权限失败、弱网降级、断线重连、隐私停止和微信小程序资质门禁。`tv-box:webrtc-sdk-gate` 会验证 Android WebRTC AAR 仍是可选依赖，默认 APK 不膨胀；当前默认候选 `io.github.webrtc-sdk:android:114.5735.11` 可用 JDK 11 编译，`125+` AAR 是 Java 17 class，需升级 JDK/AGP 后再评估；只有 `ENABLE_PHONE_CAMERA_WEBRTC=true` 的实验构建会把 minSdk 从项目基线提升到 21，并编译可选 `PhoneCameraNativeWebRtcEngine`，用于消费手机 offer、创建 answer、发送 ICE、渲染远端视频、接收音频和上报 stats。交付包会同时带 `TV_BOX_PHONE_CAMERA_CONTRACT.zh-CN.md`。电视端 `phone_camera_pair` 页面已经把二维码、6 位房间码、重新生成、返回摄像头、帮助自检、默认不录制和小程序资质门禁固化进源码审计；连上真实盒子后可运行 `BOX_IP=<盒子IP> npm run tv-box:phone-camera-pair-smoke`，自动从首页按 4 到摄像头页、再按 4 到配对页，抓取截图和 UI XML，验证页面文案、房间码、三步流程、隐私、验收边界、接收端参数传递和接收端信令状态。手机浏览器真实采集必须用安全上下文：普通 `http://局域网IP` 很可能拿不到摄像头权限，现场应使用 HTTPS/WSS 入口或手机 App；一旦手机入口是 `https://.../phone-camera`，信令必须同步为 `wss://.../phone-camera/signaling`。现场矩阵声明 `phone_webrtc` 时，`tv-box:field-import` 与 `tv-box:compatibility-summary` 会把扫码、手机权限、电视首帧、电视音频、stats、重连和停止全部作为关闭门禁；真实手机权限、电视首帧、音频和 stats 未验收前，不能把手机摄像头记为真实音视频通过。
...
仓库也提供 GitHub Actions 工作流 `.github/workflows/tv-box-check.yml`，会在 PR、main/master/codex 分支推送或手动触发时用 `package-lock.json` 执行 `npm ci --legacy-peer-deps`，再运行 `npm run tv-box:check`，并上传 `reports/tv-box-handoff/`、`reports/tv-box-handoff-latest.zip`、`reports/tv-box-field-wizard-latest.*`、`reports/tv-box-field-wizard-offline.html`、`reports/tv-box-field-import-latest.*`、`reports/tv-box-field-inbox-latest.*`、`reports/tv-box-field-inbox-imports/`、`reports/tv-box-return-inbox-latest.*`、`reports/tv-box-return-inbox-json/`、`reports/tv-box-return-inbox-scenarios-test-latest.*`、`reports/tv-box-field-scenarios-test-latest.*`、`reports/tv-box-hardware-profile-latest.*`、`reports/tv-box-c920-arrived-dry-run-latest.*`、`reports/tv-box-c920-pro-acceptance-latest.*`、`reports/tv-box-easy-run-latest.*`、`reports/tv-box-next-latest.*`、`reports/tv-box-next-scenarios-test-latest.*`、`reports/tv-box-authorization-latest.*`、`reports/tv-box-ux-audit-latest.*`、`reports/tv-box-site-readiness-latest.*`、`reports/tv-box-site-readiness-card.html`、`reports/tv-box-completion-audit-latest.*`、`reports/tv-box-release-ledger-latest.*`、`reports/tv-box-release-ledger.jsonl` 和对应 `.sha256` 作为构建产物。全量门禁会自动跑一次 `C920_ARRIVED_DRY_RUN=true` 的到货日演练，只证明日期和 ADB 前置检查，不替代真实 C920 画面、麦克风和热插拔验收。
